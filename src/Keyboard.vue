...
const rows = [
  'دجحخهعغفقثصض'.split(''),
  'ذطكمنتاأإلبيسش'.split(''),
  ['إدخال', ...'ظزوةىآرؤءئ'.split(''), 'Backspace']
]
</script>

<template>
  <div id="keyboard" dir="rtl">
    <div class="row" v-for="(row, i) in rows">
      <div class="spacer" v-if="i === 1"></div>
      <button
        v-for="key in row"
        :class="[key.length > 1 && 'big', letterStates[key]]"
        class="w-6 px-4 md:text-base"
        @click="$emit('key', key)"
      >
...
button {
  ...
  cursor: pointer;
  user-select: none;
 @apply bg-gray-300 w-9 h-9 dark:bg-gray-700 text-gray-800 dark:text-gray-200;
